// DeepseekV3Model + DeepseekV3ForCausalLM — model-level orchestration.
//
// Ports modeling_deepseek_v3.py:
//   DeepseekV3Model (1351):    embed -> N x DecoderLayer -> final RMSNorm
//   DeepseekV3ForCausalLM (1601): Model + lm_head (hidden -> vocab logits)
//
...
// embedded hidden states directly — matches the C++ KML reference which
...
const layer_mod = @import("deepseekv3_layer.zig");
...
pub const DeepseekV3Model = struct {
    config: ModelConfig,
    allocator: std.mem.Allocator,
    layers: []*layer_mod.DeepseekV3DecoderLayer,
    scratch: []amx.bf16, // ping-pong buffer: [2][max_qlen * hidden]

    pub fn init(allocator: std.mem.Allocator, config: ModelConfig) !*DeepseekV3Model {
        const layers = try allocator.alloc(*layer_mod.DeepseekV3DecoderLayer, config.num_layers);
        for (layers, 0..) |*slot, i| {
            slot.* = try layer_mod.DeepseekV3DecoderLayer.init(allocator, config.layer);
            _ = i;
        }
        const n = config.layer.max_qlen * config.layer.hidden_size;
        const self = try allocator.create(DeepseekV3Model);
        self.* = .{
            .config = config,
            .allocator = allocator,
            .layers = layers,
    ...
    pub fn deinit(self: *DeepseekV3Model) void {
    ...
    pub fn forward(
        ...
        // Final RMSNorm (modeling_deepseek_v3.py:1495) — f32 in/out with a
        // small scratch, then pack to BF16.
        const hidden = self.config.layer.hidden_size;
        const eps: f32 = 1e-6;
        var row_scratch: [4096]f32 = undefined; // hidden <= 4096 supported
...
/// Final-norm helper (mirrors mla_core.rmsNorm but sized dynamically to
/// avoid importing the whole MLA module here; identical math).
...
pub const DeepseekV3ForCausalLM = struct {
    config: CausalLMConfig,
    allocator: std.mem.Allocator,
    model: *DeepseekV3Model,
    logits_scratch: []f32, // [max_qlen * vocab_size]

    pub fn init(allocator: std.mem.Allocator, config: CausalLMConfig) !*DeepseekV3ForCausalLM {
        const model = try DeepseekV3Model.init(allocator, config.model);
        const self = try allocator.create(DeepseekV3ForCausalLM);
        self.* = .{
            .config = config,
            .allocator = allocator,
            .model = model,
    ...
    pub fn deinit(self: *DeepseekV3ForCausalLM) void {
        self.model.deinit();
