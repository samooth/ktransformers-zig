...
**Tests: WORKING** - `zig build test` RUNS all suites: 48 kernels + 11 MLA + 2 FP8 transport + 1 MLA C API = 62 total pass, 0 leaks, exit 0. (Test harness now uses a simple-mode runner, `tools/test_runner.zig` — the default Zig 0.16 `--listen=-` IPC handshake fails in this environment; see LESSONS note in build.zig.)
...
| Tests | 48 kernels (5 Q8_0 + 5 Q4_K + 4 Q6_K + 34) + 11 MLA + 2 FP8 + 1 MLA C API = 62 total pass, 0 leaks, zig build test exits 0 (simple-mode runner) | 100% |
